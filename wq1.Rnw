...
p <- show_matrix(epcdata, yrrng = c(1975, maxyr), txtsz = 5, abbrev = T, historic = T, partialyr = F) +
  theme(
...
\footnotesize
<<results = 'asis', eval = T>>=
tab <- anlz_yrattain(epcdata, yrsel = maxyr, partialyr = F) %>%
  mutate(
...
p1 <- show_thrplot(epcdata, bay_segment = "OTB", thr = "chla", yrrng = yrrng, partialyr = F) + sclx + thrthm
p1leg <- g_legend(p1)
p1 <- p1 + theme(legend.positio = 'none')
p2 <- show_thrplot(epcdata, bay_segment = "HB", thr = "chla", yrrng = yrrng, partialyr = F) + sclx + thrthm + theme(legend.position = 'none')
p3 <- show_thrplot(epcdata, bay_segment = "MTB", thr = "chla", yrrng = yrrng, partialyr = F) + sclx + thrthm + theme(legend.position = 'none')
p4 <- show_thrplot(epcdata, bay_segment = "LTB", thr = "chla", yrrng = yrrng, partialyr = F) + sclx + thrthm + theme(legend.position = 'none')
...
txtcol <- 'black'
p <- show_sitemap(epcdata, yrsel = maxyr, partialyr = F) +
  theme(
    plot.background = element_rect(fill = NA, color = NA),
