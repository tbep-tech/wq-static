...
pdf(here('figure', 'attainmat.pdf'), family = 'serif', height = 14.25, width = 4.5)
p
dev.off()
@
...
cap.val <- paste0('{\\footnotesize Observed water quality indicators \\& recommended management outcomes for ', maxyr , '.}')
cgrps <- c('Chl-a (ug/L)','\\parbox{1.5cm}{Effective Light Penetration ($m^{-1}$)}', '')
...
latex(
  ...
  colheads = c(maxyr, 'target', maxyr, 'target', '\\hspace{-0.3cm}\\parbox{0.7cm}{outcome}'),
  label = 'tab:segtab',
  col.just = rep('p{0.25in}', 6),
  table.env = F
  )
@
\end{minipage}
...
thrthm <- theme(
    ...
    axis.text.x = element_text(colour = txtcol, angle = 0, size = 10, hjust = 0.5)
  )
...
pdf(here('figure', 'thrplot.pdf'), family = 'serif', height = 7, width = 12)
grid.arrange(
  p1leg,
