...
\caption{\footnotesize Attainment of bay segments for chlorophyll criteria from 1975 to \Sexpr{maxyr} (April, May data missing for 2020).}
\label{fig:chlmat}
\end{figure}
...
pdf(here('figure', 'chlboxplot.pdf'), family = 'serif', height = 6.15, width = 8)
grid.arrange(
  p1leg,
...
\footnotesize
Tampa Bay's total seagrass coverage fell below the 40,000 acre recovery goal within the \href{https://tbep.org/habitat-master-plan-update/}{Habitat Master Plan Update} with the most recent assessment data from SWFWMD. The 2020 baywide estimate was 34,298 acres, representing a decrease of 6,354 acres from 2018 (Figure \ref{fig:sgtrnd}). Large decreases were observed in Old Tampa Bay, especially in the Feather Sound area. Increases in the attached algae \textit{Caulerpa prolifera} have also been noted in the Feather Sound region and elsewhere. Research and management plans are currently being developed to address these losses. More information on assessments of the bay's seagrass trends using transect monitoring data can be found at \href{https://shiny.tbep.org/seagrasstransect-dash/}{https://shiny.tbep.org/seagrasstransect-dash/} and using the coverage estimates from SWFWMD can be found at \href{https://shiny.tbep.org/seagrass-analysis/}{https://shiny.tbep.org/seagrass-analysis/}.
\end{minipage}
\hspace{0.2in}
